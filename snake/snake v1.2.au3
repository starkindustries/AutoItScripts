	...
	ElseIf($dir == $down) Then
		...
		Else ; else if kills is greater than 20 do this:			
			...
			While($contLoop)
				...
				If squareIsGray(0,0) Then ; if top left square is gray
					ToolTip("Grid(0,0) is GRAY!")
					; then exit the loop
					$contLoop = False
				EndIf
...
Func squareIsGray($x, $y)
	...
	$y = $pxpos[1]
	; MouseMove($pixelPos[0], $pixelPos[1], 0) ; for Debug purposes
	PixelSearch($x-$dy, $y-$dy, $x+$dy, $y+$dy, $gray)
	If Not @error Then ; if gray found then return True
		Return True
	Else
		Return False
	EndIf
