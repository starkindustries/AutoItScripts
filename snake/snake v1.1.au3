...
While(True)
	...
	ElseIf($dir == $left) Then		
		...
		If Not @error And ($gbox[0] < $gpos[0]) And Not($gbox[0] == 1)  Then
			; move x-pos ahead one step to account for searchGridForRed() delay
			$gpos[0] = $gpos[0] - 1 
			$dest[0] = $gbox[0]
			$dest[1] = $gpos[1]
		...
		ElseIf Not @error And ($gbox[0] < $gpos[0]) And ($gbox[0] == 1) Then
			...
			ToolTip("UP")
			$dir = $up			
		Else ; else (no redboxes to target) wait till snake is at edge, then go up
			; move x-pos ahead one step to account for searchGridForRed() delay
			$gpos[0] = $gpos[0] - 1
			$dest[0] = 0 ; leftLimit
			$dest[1] = $gpos[1]
...
Func waitForSnake($src, $dest, $dir)	
	...
	If($dir == $left Or $dir == $right) Then
		For $i = $init to $diff Step $step
			...
			$pos = waitTillBlueFound($pxpos[0]-$dy, $pxpos[1]-$dy, $pxpos[0]+$dy, $pxpos[1]+$dy)
		Next
...
Func searchGridForRed()
	...
	If Not($gpos[0] == $oldbox[0] And $gpos[1] == $oldbox[1]) Then
		...
		ToolTip($kills & " kills")
	EndIf
	; check if the redbox is along the outer edge
