...
; check if single or dual screen
If($singlescreen) Then
	; for single screen: top-left = (275,454)
	$leftLimit  = 275
	$topLimit   = 454
	; for single screen: bot-right = (786, 710)	
	$rightLimit = 786
	$botLimit   = 710
Else
...
While(True)	
	...
	ElseIf($dir == $right) Then		
		...
		Local $trl = ($totalNumBoxesX-1) - ($loops*2 + 2) - 1
		If($redbox[0] == 0) Or ($redbox[0] == $totalNumBoxesX-1) Or ($redbox[1] == 0) Or ($redbox[1] == $totalNumBoxesY-1) Then
			...
			ToolTip("DOWN")
			$dir = $down
		; check if redbox is at the 2nd right-most column
		; if it is then continue guiding along the wall's edge
		ElseIf($redbox[0] == ($totalNumBoxesX-2)) Then		
			ToolTip("redbox on 2nd right-most column!")
			$dest[0] = $totalNumBoxesX - 1 ; rightLimit
			$dest[1] = $gpos[1]
			$gpos = waitForSnakeToGoRight($gpos, $dest)
			Send("{DOWN}")
...
Func waitTillBlueFound($x0, $x1, $x2, $x3)
	...
	While(@error) ; while blue is NOT found, search again
		; Sleep($sleep)
		$pos = PixelSearch($x0, $x1, $x2, $x3, $blue) ; TODO mod to calibrate with wherever the screen may be
		; Local $diff = TimerDiff($begin) ; TODO modify to account for a delay > 100		
	WEnd
	; Local $diff = TimerDiff($begin)
	; If($diff > $maxdelay) Then
	; 	$maxdelay = $diff
	; EndIf	
EndFunc
...
Func exitProgram()
	; MsgBox(0, "Max delay", "Max delay" & $maxdelay) ; maxdelay so far: 167ms
	Exit
EndFunc
